...
class MenuScreen(AbstractScreen):
    ...
    def draw(self, surface):
        ...
        label_p = self.font.render(("> " if self.bot_select == 0 else "  ") + "PLAYER 1: " + list(self.bot_list.keys())[self.bot_p1], 1, self.COLOR_WHITE)
        label_p_rect = label_p.get_rect()
        label_p_rect.center = rect.centerx, rect.height*18/40
        surface.blit(label_p, label_p_rect)
        label_p = self.font.render(("> " if self.bot_select == 1 else "  ") + "PLAYER 2: " + list(self.bot_list.keys())[self.bot_p2], 1, self.COLOR_WHITE)
        label_p_rect = label_p.get_rect()
    ...
    def event(self, event):
        if event.type == QUIT:
            ...
            if event.key == K_ESCAPE:
                ...
                self.action(self.ACTION_SETP1, self.bot_list[list(self.bot_list.keys())[self.bot_p1]])
                self.action(self.ACTION_SETP2, self.bot_list[list(self.bot_list.keys())[self.bot_p2]])
